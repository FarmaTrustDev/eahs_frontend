<template>
        ...
        >
          <template slot="shelves" slot-scope="shelves">
            <span>
              ...
              <span :style="{color:'blue',fontSize: '15px'}" @click="goto('/administration/storage/facility/storageConfig/newFridge?fId='+shelves.globalId+'&approval='+false+'&view='+true)">View Detail</span>
              </a-tooltip>
            </span>
          </template>
  ...
  export default {
    ...
    methods: {
      ...
      fetchFridge(){
        FridgeServices.getRejected().then((response)=>{
